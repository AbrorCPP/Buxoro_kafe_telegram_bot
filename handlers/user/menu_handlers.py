...
async def show_categories(call: CallbackQuery):
    ...
    await call.message.answer("Kategoriyani tanlang:", reply_markup=builder.as_markup())
...
async def show_product_detail(call: CallbackQuery):
    try:
        ...
        builder.button(text="🔙 Orqaga qaytish", callback_data=f"cat_{product['category_id']}")
        builder.adjust(1)
...
async def back_to_main(call: CallbackQuery):
    ...
    await call.message.delete()
    await call.message.answer("Asosiy menyu:", reply_markup=main_menu_inline())
