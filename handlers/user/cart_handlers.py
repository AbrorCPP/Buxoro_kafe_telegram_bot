...
async def show_cart_items(call: CallbackQuery):
    ...
    if not cart_items:
        await call.answer("Savatchangiz bo'sh 🛒", show_alert=True)
        return
    ...
    for item in cart_items:
        ...
        total_price += price
        text += f"▫️ {item['name']} x {item['quantity']} = {price} so'm\n"
        
    text += f"\nJami: {total_price} so'm"
    
    builder.button(text="Tasdiqlash va Buyurtma qilish ✅", callback_data="checkout")
...
async def process_clear_cart(call: CallbackQuery):
    ...
    await call.message.delete()
    await call.message.answer("Savatcha tozalandi 🗑", reply_markup=main_menu_inline())
...
async def process_checkout(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer("Iltimos, yetkazib berish manzilini joylashuv orqali yuboring (yoki matn shaklida yozing):", reply_markup=kb)
...
async def process_checkout_location(message: Message, state: FSMContext):
    ...
    if message.location:
        ...
        loc = message.text
        # Fallback pseudo-coordinates if text given
    ...
    try:
        ...
        await message.answer(f"Joylashuv qabul qilindi! Masofa: {distance:.1f} km.\nTo'lovni amalga oshiring:", reply_markup=ReplyKeyboardRemove())
        await bot.send_invoice(
            chat_id=message.chat.id,
            title=f"Buxoro Kafe: Buyurtma #{order_id}",
            description=f"Ovqat narxi: {total_price} so'm\nYetkazish ({distance:.1f} km): {delivery_price} so'm",
        ...
        from keyboards.reply.main_menu import main_menu_inline
        await message.answer(f"Buyurtmangiz tasdiqlandi (Lekin To'lov tizimi integratsiyasi sozlanmagan). ✅\nBuyurtma: #{order_id}", reply_markup=main_menu_inline())
        
        # Test uchun: to'lovsiz ham kuryerlarga tashlab yuboramiz
        from loader import bot
        from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        couriers = db.get_all_couriers()
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🙋‍♂️ Men yetkazaman!", callback_data=f"volunteer_{order_id}")]
        ])
        for c in couriers:
            try:
                await bot.send_message(c['telegram_id'], f"Yangi buyurtma! #{order_id}\n\nQabul qilasizmi?", reply_markup=kb)
...
async def process_successful_payment(message: Message):
    ...
    if payload and payload.startswith("order_"):
        ...
        for c in couriers:
            try:
                await bot.send_message(c['telegram_id'], f"🎉 TO'LOV QILINGAN Yangi buyurtma! #{order_id}\n\nOlamizmi?", reply_markup=kb)
            except:
                pass

    await message.answer("To'lovingiz muvaffaqiyatli qabul qilindi! 🎉 Tayyorgarlik boshlandi.", reply_markup=main_menu_inline())
